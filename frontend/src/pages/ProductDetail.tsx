...
const DirectBuyPanel = ({ product }: { product: Product }) => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8">
      ...
      <StatusBadge
        type="stock"
        status={product.inStock !== false ? "inStock" : "outOfStock"}
      />
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8 text-center">
      ...
      <p className="text-gray-600 mb-6">
        Contact us for custom pricing and bulk availability.
      </p>
      <button
...
const AuctionPanel = ({
  ...
  product: Product;
  onBidSuccess: (updatedProduct: Product) => void;
}) => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (now < startTime) {
        ...
        setTimeLeft(
          `Starts in: ${Math.floor(diff / (1000 * 60 * 60 * 24))}d ${Math.floor(
            (diff / (1000 * 60 * 60)) % 24
  ...
  const handlePlaceBid = async () => {
    ...
    if (!bidAmount || bidAmount <= (product.currentPrice || 0)) {
      setError(
        `Your bid must be higher than the current price of ₹${product.currentPrice}.`
      );
      return;
    }
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/products/${product._id}/bids`,
        {
          method: "POST",
      ...
      <div className="space-y-3 mb-6">
        <div className="flex justify-between items-center">
          ...
          <span className="text-2xl font-bold text-green-600">
            ₹{product.currentPrice || product.startingBid}
          </span>
        </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder={`Minimum ₹${
                  (product.currentPrice || product.startingBid)! + 1
                }`}
...
const ProductDetail = () => {
  ...
  const renderActionPanel = (product: Product) => {
    switch (product.buyType) {
      ...
      case "auction":
        return (
          <AuctionPanel product={product} onBidSuccess={handleBidSuccess} />
        );
      default:
        return null;
    }
            ...
            <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
              <img
                src={
                  ...
                  "https://placehold.co/800x800/EEE/31343C?text=No+Image"
                }
                alt={product.name}
          ...
          <div className="space-y-6">
            <div>
              <div className="flex items-center gap-3 mb-3">
                <StatusBadge
                  type={product.buyType}
                  status={product.buyType === "auction" ? "Live" : undefined}
                />
                {product.organic && <StatusBadge type="organic" />}
              </div>
